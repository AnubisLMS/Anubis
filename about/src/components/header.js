        ...
        <div className="flex flex-col w-full items-start justify-center  space-y-4">
          {links.map((link, index) => (
            <div className="border-b-2 border-light-500 w-full pb-4 ">
              <HeaderLink key={`${link.name}-${index}`} to={link.path}>
                {link.name}
              </HeaderLink>
            </div>
          ))}
  ...
  return (
    <div className="flex flex-col w-full p-4">
      <div className="flex flex-row w-full h-20  justify-between items-center">
        ...
        {screenSize === 'lg' && (
          <a href = "https://anubis.osiris.services/api/public/auth/login" className="space-x-2">
            <SecondaryButton>Log In</SecondaryButton>
          </a>
        )}
        ...
        {screenSize !== 'lg' && (
          <div
            ...
            className="bg-gray-800 rounded-full pr-4 pl-4 pt-2 pb-2 hover:bg-gray-700 cursor-pointer"
          >
