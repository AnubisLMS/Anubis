...
def compare_test_fixtures() -> List[CompareTestFixture]:
    return [
        ...
        CompareTestFixture(
            ...
            diff=["--- ", "+++ ", "@@ -1,2 +1,1000 @@", "-foo", "-bar"]
            + ["+my broken for loop" for _ in range(1000)],
        ),
    ]
...
def test_compare_func(compare_test_fixtures: List[CompareTestFixture]):
    for fixture in compare_test_fixtures:
        matched, diff = fixture.comp_func(
            fixture.actual, fixture.expected, fixture.case_sensitive
        )
...
trim_test_cases = [
    TrimTestCase(
        "trailing timeout at the same line",
...
Test2
Test3 
Test4$ qemu-system-i386: terminating on signal 15 from pid 1454 (timeout)""",
        ["Test1", "Test2", "Test3", "Test4"],
    ),
    TrimTestCase(
        "trailing timeout at a separate line",
        """Booting from Hard Disk..xv6...
cpu0: starting
myprog test.txt
sb: size 1000 nblocks 941 ninodes 200 nlog 30 logstart 2 inodestart 32 bmap start 58
...
@pytest.mark.parametrize(
    "test_case", trim_test_cases, ids=lambda test_case: test_case.name
)
