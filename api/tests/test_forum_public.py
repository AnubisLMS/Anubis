...
def test_post_create():
    ...
    # Get post
    data = s.get(f'/public/forums/post/{post1_id}')
    assert data
...
def test_post_create_edit():
    ...
    # Edit post
    data = s.patch_json(f'/public/forums/post/{post1_id}', {
        'title': 'title2',
        'content': 'content2',
        'visible_to_students': True,
        'anonymous': False,
...
def test_post_create_anon():
    ...
    data = s2.get(f'/public/forums/post/{post1_id}')
    assert data
    assert data['post']['title'] == 'title1'
    assert data['post']['content'] == 'content1'
...
def test_post_delete():
    ...
    # Delete post
    data = s.delete(f'/public/forums/post/{post1_id}')
    assert data
    assert data['status'] == 'Post deleted'

    # Get post
    s.get(f'/public/forums/post/{post1_id}', should_fail=True)
