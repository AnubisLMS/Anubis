...
def test_static_public():
    ...
    blob_id = prof.post("/admin/static/upload", files={filename: logo_file})["blob"][
        "path"
    ].lstrip("/")
    ...
    r = student.get(
        f"/public/static/{blob_id}/logo.png", return_request=True, skip_verify=True
    )
    assert r.status_code == 200
    assert r.headers.get("content-type") == "image/png"

    r = student.get(
        f"/public/static/{blob_id}/logo.pn", return_request=True, skip_verify=True
    )
